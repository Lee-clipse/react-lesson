...
function Index0906() {
  ...
  useEffect(() => {
    const result = requestProductData();
  ...
  function handleCheckProduct(product) {
    ...
    if (isSelectedProduct) {
      ...
      const newProductList = selectedProducts.filter((item) => {
        if (item === product) {
          return false;
        } else {
          return true;
  ...
  return (
    <div>
      ...
      <div id="lesson-content">
        ...
        <div>
          4. `결제하기` 버튼을 누르면 체크한 상품들의 가격 합이 계산된다.
        </div>
        <div>
          5. 가격 합이 10000원보다 작다면 결제 성공, 그렇지 않다면 결제가
          실패한다.
        </div>
      </div>
