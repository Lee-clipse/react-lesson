...
function Index1010() {
  ...
  function shoppingCartInput(product, qty) {
    setProductModal(false);
    setShoppingCart([...shoppingCart, { name: product.name, qty: qty, price: product.price }]);
  }
    ...
    <>
      {productModal && (
        <ProductModal2
          ...
          shoppingCartInput={(product, qty) => shoppingCartInput(product, qty)}
        />
      )}
      ...
      <div>
        ...
        <div id="lesson-content">
          ...
          {productList.map((product) => {
            return (
              <div className="link-button" onClick={() => selectProduct(product)}>
                {product.id}번 상품 선택
              </div>
            );
          })}
